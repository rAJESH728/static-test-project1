...
function SmallComp(props) {
    return ( 
        <div className="w-80 px-2 py-4 " >
            ...
            <p className={`inline-block text-xs bg-${props.onTabAuthorColor}-100 text-${props.onTabAuthorColor}-500 rounded-sm m-5 p-2`}>{props.onTabAuthor}</p>
            </div>
            <div className="bg-white border-2 drop-shadow-lg border-t-0 rounded-b-lg p-4">
                <MdOutlineAttachFile className={`inline-block text-xl ${props.onTabAttachColor.length<=0?'text-gray-100':'text-gray-500'}`}/>
                <p className={`inline-block text-s pl-1 text-gray-500`}>{props.onTabAttach}</p>
                <BsFillFlagFill className={`inline-block ml-5 text-${props.onTabFlagColor}-600 `} />
